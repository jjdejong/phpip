...
<body>
    ...
    @foreach ($matters as $matter)
        ...
        <tr>
            ...
            <td>{!! $matter['FilNo'] !!}<br /><span class="tdate">{!! empty($matter['Filed'])? "" : Carbon\Carbon::parse($matter['Filed'])->isoFormat('LL')  !!}</span></td>
                <td>{!! $matter['PubNo'] !!}<br /><span class="tdate">{!! empty($matter['Published'])? "" : Carbon\Carbon::parse($matter['Published'])->isoFormat('LL') !!}</span></td>
                <td>{!! $matter['GrtNo'] !!}<br /><span class="tdate">{!! empty($matter['Granted'])? "" : Carbon\Carbon::parse($matter['Granted'])->isoFormat('LL') !!}</span></td>
                <td>
                    @if (  $matter['dead'] == 1 )
                        Fermé : {!! $matter['Status'] !!}
                    @else
                ...
                <td><span class="tdate">{!! empty($matter['next'])? "" : Carbon\Carbon::parse($matter['next'])->isoFormat('LL') !!}</span></td>
            <td>{!! $matter['Ref'] !!}</td>
        </tr>
        @endif
    @endforeach
