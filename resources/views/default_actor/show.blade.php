<div data-resource="/default_actor/{{ $default_actor->id }}">
  <table class="table">
    <tr>
      ...
      <td><input type="text" class="noformat form-control" name="role" data-ac="/role/autocomplete" value="{{ empty($default_actor->roleInfo) ? '' : __($default_actor->roleInfo->name) }}" autocomplete="off"></td>
    </tr>
    <tr>
      <th><label title="{{ $tableComments['for_country'] }}">{{ __('Country') }}</label></th>
      <td><input type="text" class="noformat form-control" name="for_country" data-ac="/country/autocomplete" value="{{ empty($default_actor->country) ? '' : __($default_actor->country->iso) }}" autocomplete="off"></td>
      <th><label title="{{ $tableComments['for_category'] }}">{{ __('Category') }}</label></th>
      <td><input type="text" class="noformat form-control" name="for_category" data-ac="/category/autocomplete" value="{{ empty($default_actor->category) ? '' : __($default_actor->category->category) }}" autocomplete="off"></td>
    </tr>
    <tr>
  ...
  <button type="button" class="btn btn-danger" title="{{ __('Delete entry') }}" id="deleteDActor" data-message="entry for {{ $default_actor->actor->name  }}" data-url='/default_actor/{{ $default_actor->id }}'>
    {{ __('Delete') }}
  </button>
</div>
