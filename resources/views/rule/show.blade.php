...
<div id="edit-rule-content">
	...
	<fieldset>
		...
		<table class="table table-sm table-hover" data-id="{{ $ruleInfo->id }}">
                <tr><td><label for="task" class="required-field" title="{{ $ruleComments['task'] }}">Task</label> 
                </td><td class="ui-front"><input class="rule-input-wide noformat" name="task" value="{{ empty($ruleInfo->taskInfo) ? '' : $ruleInfo->taskInfo->name }}">
                		...
                		<input type="text" class="rule-input-wide noformat" name="for_country" value="{{ empty($ruleInfo->country) ? '' : $ruleInfo->country->name }}">
                </td><td><label for="is_active" title="{{ $ruleComments['active'] }}">Is active</label>
                </td><td><span class="rule-input-narrow noformat" name="active">
                		...
                		<input class="rule-input-wide noformat" name="for_origin" value="{{ empty($ruleInfo->origin) ? '' : $ruleInfo->origin->name }}">
                </td><td><label for="for_category" title="{{ $ruleComments['for_category'] }}">Category</label>
                </td><td class="ui-front">
						<input class="rule-input-narrow noformat" name="for_category" value="{{ empty($ruleInfo->category) ? '' : $ruleInfo->category->category }}">
					...
					<input class="rule-input-narrow noformat" name="for_type" value="{{ empty($ruleInfo->type) ? '' : $ruleInfo->type->type }}">
				</td></tr>
				</table>
        </fieldset>
        <fieldset>
                ...
                <tr><td><label for="trigger_event" title="{{ $ruleComments['trigger_event'] }}">Trigger event</label>
					...
					<input class="rule-input-wide noformat" name="trigger_event" value="{{ empty($ruleInfo->trigger) ? '' : $ruleInfo->trigger->name }}">
                </td><td><label for="use_parent" title="{{ $ruleComments['use_parent'] }}">Use parent</label>
                </td><td><span class="rule-input-narrow noformat " name="use_parent">
                ...
                </td><td><span class="rule-input-narrow noformat " name="clear_task">
                ...
                </td><td><input type='text' class="rule-input-wide noformat noformat form-control" size="10" name="use_after" value="{{ $ruleInfo->use_after != "" ?  $ruleInfo->use_after: "..." }}">
                </td><td><label for="delete_task" title="{{ $ruleComments['delete_task'] }}">Delete task</label> 
                </td><td><span class="rule-input-narrow noformat " name="delete_task">
						...
						<input class="rule-input-wide noformat" name="condition_event" value="{{ empty($ruleInfo->condition_eventInfo) ? '' : $ruleInfo->condition_eventInfo->name }}">
				</td><td><label for="use_priority" title="{{ $ruleComments['use_priority'] }}">Use priority</label>
                </td><td><span class="rule-input-narrow noformat " name="use_priority">
						...
						<input class="rule-input-wide noformat" name="abort_on" value="{{ empty($ruleInfo->abort_onInfo) ? '' : $ruleInfo->abort_onInfo->name }}">
                </td><td><label type='text' for="responsible" title="{{ $ruleComments['responsible'] }}">Responsible</label>
                </td><td class="ui-front">
						<input class="rule-input-narrow noformat noformat" name="responsible" value="{{ empty($ruleInfo->responsibleInfo) ? '' : $ruleInfo->responsibleInfo->name }}">
                ...
                </td><td><span class="rule-input-narrow noformat" name="end_of_month">
