<form id="createMatterForm" autocomplete="off">
  ...
  <div class="row mb-2">
    <label for="category" class="col-4 col-form-label fw-bold">{{ __('Category') }}</label>
    <div class="col-8">
      <input type="hidden" name="category_code" value="{{ $parent_matter->category_code ?? ( $category['code'] ?? '') }}">
      <input type="text" class="form-control" data-ac="/category/autocomplete" data-actarget="category_code" data-aclength="0" placeholder="{{ $category['name'] ?? ( __($parent_matter->category->category) ??  '' ) }}" autocomplete="off">
    </div>
  </div>
  @if ( $operation == 'ops' )
  <div class="row mb-2">
    <label for="docnum" class="col-4 col-form-label fw-bold">{{ __('Pub Number') }}</label>
    <div class="col-8">
  ...
  @if ( $operation == 'child' )
  <fieldset>
    <legend>{{ __('Use current :category as:', ['category' => __($parent_matter->category->category ?? 'matter') ])  }}</legend>
