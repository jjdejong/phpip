<div data-resource="/classifier_type/{{ $classifier_type->code }}">
	<table class="table">
		<tr>
			...
			<th><label title="{{ __($tableComments['type']) }}">{{ __('Name') }}</label></th>
			<td><input class="form-control noformat" name="type" value="{{ $classifier_type->type }}"></td>
		</tr>
		<tr>
			<th><label title="{{ __($tableComments['display_order']) }}">{{ __('Display order') }}</label></th>
			<td><input class="form-control noformat" type='text' name="display_order" value="{{ $classifier_type->display_order }}"></input>
			<th><label title="{{ __($tableComments['for_category']) }}">{{ __('Category') }}</label></th>
		...
		<tr>
      <th><label title="{{ __($tableComments['main_display']) }}">{{ __('Main display') }}</label></th>
	...
	<button type="button" class="btn btn-danger" title="{{ __('Delete type') }}" id="deleteClassifierType" data-message="type {{$classifier_type->type }}" data-url='/classifier_type/{{ $classifier_type->code }}'>
		{{ __('Delete') }}
	</button>
</div>
