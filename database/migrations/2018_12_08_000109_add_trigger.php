...
class AddTrigger extends Migration
{
    ...
    public function up()
    {   // For actor
        DB::unprepared("CREATE `actor_creator_log` BEFORE INSERT ON `actor` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `actor_updater_log` BEFORE UPDATE ON `actor` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1);");

        // For actor_role
        DB::unprepared("CREATE `arole_create_log` BEFORE INSERT ON `actor_role` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `arole_modify_log` BEFORE UPDATE ON `actor_role` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1)");

        // For classifier
        DB::unprepared("CREATE `classifier_BEFORE_INSERT` BEFORE INSERT ON `classifier` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared( "CREATE `classifier_updater_log` BEFORE UPDATE ON `classifier` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1);");

        // For classifier_type
        DB::unprepared("CREATE `ctype_creator_log` BEFORE INSERT ON `classifier_type` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `ctype_updater_log` BEFORE UPDATE ON `classifier_type` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1);");

        // For classifier value
        DB::unprepared("CREATE `cvalue_creator_log` BEFORE INSERT ON `classifier_value` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `cvalue_updater_log` BEFORE UPDATE ON `classifier_value` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1)");

        // For event
        DB::unprepared("CREATE `event_before_insert` BEFORE INSERT ON `event` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared("CREATE `event_after_insert` AFTER INSERT ON `event` FOR EACH ROW
          trig: BEGIN
        ...
        DB::unprepared("CREATE `event_before_update` BEFORE UPDATE ON `event` FOR EACH ROW
          BEGIN
          	DECLARE vdate DATE DEFAULT NULL;
        ...
        DB::unprepared("CREATE `event_after_update` AFTER UPDATE ON `event` FOR EACH ROW
          trig: BEGIN
        ...
        DB::unprepared("CREATE `event_after_delete` AFTER DELETE ON `event` FOR EACH ROW
          BEGIN
        ...
        // For event_name
        DB::unprepared("CREATE `ename_before_insert` BEFORE INSERT ON `event_name` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `ename_before_update` BEFORE UPDATE ON `event_name` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `ename_after_update` AFTER UPDATE ON `event_name` FOR EACH ROW
          BEGIN
        ...
        // For matter
        DB::unprepared("CREATE `matter_before_insert` BEFORE INSERT ON `matter` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `matter_after_insert` AFTER INSERT ON `matter` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared("CREATE `matter_before_update` BEFORE UPDATE ON `matter` FOR EACH ROW
          BEGIN
            set new.updater=SUBSTRING_INDEX(USER(),'@',1);
        ...
        DB::unprepared("CREATE `matter_after_update` AFTER UPDATE ON `matter` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared("CREATE `malnk_before_insert` BEFORE INSERT ON `matter_actor_lnk` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `malnk_after_insert` AFTER INSERT ON `matter_actor_lnk` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared("CREATE `malnk_before_update` BEFORE UPDATE ON `matter_actor_lnk` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1)" );

        DB::unprepared("CREATE `matter_actor_lnk_AFTER_UPDATE` AFTER UPDATE ON `matter_actor_lnk` FOR EACH ROW
          BEGIN
          	UPDATE matter SET updated = Now(), updater = SUBSTRING_INDEX(USER(),'@',1) WHERE matter.id=NEW.matter_id;
          END"
        );

        DB::unprepared("CREATE `matter_actor_lnk_AFTER_DELETE` AFTER DELETE ON `matter_actor_lnk` FOR EACH ROW
          BEGIN
          	UPDATE matter SET updated = Now(), updater = SUBSTRING_INDEX(USER(),'@',1) WHERE matter.id=OLD.matter_id;
          END"
        );
        ...
        // For matter_category
        DB::unprepared("CREATE `mcateg_creator_log` BEFORE INSERT ON `matter_category` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `mcateg_updater_log` BEFORE UPDATE ON `matter_category` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `mtype_creator_log` BEFORE INSERT ON `matter_type` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `mtype_updater_log` BEFORE UPDATE ON `matter_type` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1)");

        // For tasks
        DB::unprepared("CREATE `task_before_insert` BEFORE INSERT ON task FOR EACH ROW
          BEGIN
        ...
        // For task_rules
        DB::unprepared("CREATE `trules_before_insert` BEFORE INSERT ON `task_rules` FOR EACH ROW set new.creator=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `trules_before_update` BEFORE UPDATE ON `task_rules` FOR EACH ROW set new.updater=SUBSTRING_INDEX(USER(),'@',1)");

        DB::unprepared("CREATE `trules_after_update` AFTER UPDATE ON `task_rules` FOR EACH ROW
          BEGIN
