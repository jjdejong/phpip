...
class AddTrigger extends Migration
{
    ...
    {   // For actor
        ...
        // For event
        DB::unprepared("CREATE DEFINER = `root`@`localhost` TRIGGER `event_before_insert` BEFORE INSERT ON `event` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared("CREATE DEFINER = `root`@`localhost` TRIGGER `event_before_update` BEFORE UPDATE ON `event` FOR EACH ROW
          BEGIN
          	DECLARE vdate DATE DEFAULT NULL;
        ...
        DB::unprepared("CREATE DEFINER = `root`@`localhost` TRIGGER `ename_after_update` AFTER UPDATE ON `event_name` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared("CREATE DEFINER = `root`@`localhost` TRIGGER `matter_after_insert` AFTER INSERT ON `matter` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared("CREATE DEFINER = `root`@`localhost` TRIGGER `matter_before_update` BEFORE UPDATE ON `matter` FOR EACH ROW
          BEGIN
            set new.updater=SUBSTRING_INDEX(USER(),'@',1);
        ...
        DB::unprepared("CREATE DEFINER = `root`@`localhost` TRIGGER `matter_after_update` AFTER UPDATE ON `matter` FOR EACH ROW
          BEGIN
        ...
        DB::unprepared("CREATE DEFINER = `root`@`localhost` TRIGGER `trules_after_update` AFTER UPDATE ON `task_rules` FOR EACH ROW
          BEGIN
