...
class CreateActorTable extends Migration {
	...
	public function up()
	{
		Schema::create('actor', function(Blueprint $table)
		{
			...
			$table->boolean('phy_person')->nullable()->default(1)->comment('Physical person or not');
			$table->char('nationality', 2)->nullable()->index('nationality');
			$table->boolean('small_entity')->nullable()->default(0)->comment('Small entity status used in a few countries (FR, US)');
			$table->string('address', 256)->default('')->comment('Main address: street, zip and city');
			$table->char('country', 2)->nullable()->index('country')->comment('Country in address');
			$table->string('address_mailing', 256)->default('')->comment('Mailing address: street, zip and city');
			$table->char('country_mailing', 2)->nullable()->index('country_mailing');
			$table->string('address_billing', 256)->default('')->comment('Billing address: street, zip and city');
			...
			$table->boolean('warn')->nullable()->default(0)->comment('The actor will be displayed in red in the matter view when set');
